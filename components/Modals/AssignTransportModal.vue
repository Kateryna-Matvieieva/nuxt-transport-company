...
export default {
  ...
  methods: {
    ...
    async fetchAvailableTransports(routeId) {
      this.fetching = true;
      try {
        const response = await this.$axios.$get(`${this.$config.apiUrl}/api/v1/routes/${routeId}/availableTransports`, {
          headers: {
    ...
    async confirmAssignment() {
      if (this.selectedTransportId) {
        this.fetching = true;
        try {
          const response = await this.$axios.$patch(`${this.$config.apiUrl}/api/v1/routes/${this.routeId}/assignTransport/${this.selectedTransportId}`, {}, {
            headers: {
              'x-api-key': this.$config.apiKey,
            },
          });
